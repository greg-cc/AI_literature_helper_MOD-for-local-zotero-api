...
def query_ollama_chat(model, prompt, temperature=0.3):
    ...
    Includes a 5-MINUTE RETRY LOOP for timeouts or connection errors.
    """
    ...
    # --- RETRY CONFIGURATION ---
    GLOBAL_TIMEOUT_BUDGET = 300  # 5 Minutes total to get a result
    MAX_GEN_TIME_PER_ATTEMPT = 180 # If one specific try takes > 3 mins, kill it and retry
        ...
        try:
            ...
            with requests.post(url, json=payload, stream=True, timeout=10) as r:
                ...
                for line in r.iter_lines():
                    ...
                    # 1. CHECK: Has this specific attempt stalled?
                    if (time() - attempt_start_time) > MAX_GEN_TIME_PER_ATTEMPT:
                        print(f"\n\n⚠️ ATTEMPT STALLED: Generation exceeded {MAX_GEN_TIME_PER_ATTEMPT}s. Restarting Ollama query...")
                        break # Break the inner loop to trigger the outer while loop (Retry)

                    # 2. CHECK: Have we exceeded the 5-minute global budget?
                    if (time() - global_start_time) > GLOBAL_TIMEOUT_BUDGET:
                        print(f"\n\n🚨 GLOBAL TIMEOUT: Exceeded {GLOBAL_TIMEOUT_BUDGET}s total retry budget.")
                        return "ERROR: OLLAMA TIMEOUT (GAVE UP)"
            ...
            # If we finished successfully, return the result
            if generation_complete:
                print(f"\n{'='*20} [STREAM END] {'='*20}\n")
                return full_response
            
            # If we exited the loop but generation_complete is False, it means we broke due to stall
            ...
            if remaining > 0:
                print(f"♻️ Retrying in 5 seconds... ({int(remaining)}s budget remaining)")
                sleep(5)
            else:
                return f"OLLAMA ERROR: {e}"

    return "ERROR: OLLAMA TIMEOUT (5 MINUTE LIMIT EXCEEDED)"
...
def _display_cycle_header(item):
    """Renders the Cycle Header or Separator immediately."""
    
    # TYPE 1: THICK BLACK LINE SEPARATOR
...
def process_chunk_and_save(chunk, query, total_papers, papers_processed, status_ph, prog_ph, query_stats, collection_override=None, vec_min_override=None, composite_min_override=None, extra_semantic_sentence=None, start_rec=0, stop_rec=0):
    ...
    for i, paper in enumerate(chunk):
        ...
        if "API FAILURE" in ai_abs:
            status_msg = "SKIPPED"
            reason = f"AI Analysis Failed (No Score)"
        elif is_dup:
        ...
        if not is_dup and not bypass_ai:
            # --- LOGIC FIX: Don't count API Failures towards Fail Fast ---
            if "API FAILURE" not in ai_abs:
...
def run_cycle_logic(queries):
    ...
    for q_idx in range(start_q_idx, len(queries)):
        ...
        _display_cycle_header(header_entry) # <--- IMMEDIATE RENDER
        # -------------------------------------
...
if st.session_state.results_history:
    for idx, item in enumerate(st.session_state.results_history):
        ...
        # Use the helper for Headers and Separators
        if item.get('type') in ['separator', 'cycle_header']:
            _display_cycle_header(item)
            
        # Use the detail renderer for Papers
        else:
